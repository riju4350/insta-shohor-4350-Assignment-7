...
const displayLikedPosts = () => {
  ...
  likedPosts.forEach((post) => {
    const div = createPost(post);
    likedItems.appendChild(div);
  });
};
/*  document.getElementById("liked") ke innerHTML diye clear korle heading tao remove hoye jay. tai ekta extra div create kore oita ke clear korchi */

const displayReportedPosts = () => {
  const reportedItems = document.getElementById("reportedItems");
  reportedItems.innerHTML = "";
  const reportedPosts = getReportedPosts();
  reportedPosts.forEach((post) => {
    const div = createPost(post);
    reportedItems.appendChild(div);
  });
};
